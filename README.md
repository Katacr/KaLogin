...
KaLogin 支持两种认证模式，可根据服务器需求自由选择：

- **AuthMe 模式**：KaLogin 提供登录/注册等 UI 界面与交互体验，底层账号认证、注册数据与登录状态判断仍由 **AuthMe** 完成。
- **KaLogin 模式**：KaLogin 使用自己的数据库与认证系统，独立完成注册、登录、密码校验等完整流程。

如果你已经在使用 AuthMe，但希望拥有更现代化、更可自定义的登录注册界面，可以选择 **AuthMe 模式**；如果你希望直接使用 KaLogin 的完整账号系统，则可以使用 **KaLogin 模式**。
